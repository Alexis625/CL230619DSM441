...
class TeacherUploadPdf : AppCompatActivity() {
    ...
    private fun selectPdfFiles() {
        ...
        startActivityForResult(Intent.createChooser(intent, "Seleccione PDF"), PDF_REQUEST_CODE)
    }

    private fun postToFirebase() {
        if (pdfUris.isEmpty()) {
            Toast.makeText(this, "Por favor seleccione al menos un archivo PDF", Toast.LENGTH_SHORT).show()
            return
        }
