...
class AttendenceActivity : AppCompatActivity() {
    ...
    private fun submitAttendance() {
        ...
        for (i in 0 until userListView.count) {
            ...
            val status = if (presentRadioButton.isChecked) "Presente" else "Ausente"
        ...
        divisionAttendanceRef.setValue(attendanceMap)
            .addOnSuccessListener {
                Toast.makeText(this, "Asistencia agregada correctamente", Toast.LENGTH_SHORT).show()
            }
            .addOnFailureListener { e ->
                Toast.makeText(this, "No se pudo enviar la asistencia: ${e.message}", Toast.LENGTH_SHORT).show()
            }
    }
